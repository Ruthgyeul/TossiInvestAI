...
발생시키지 않는다 (CLAUDE.md 절대 규칙 5 — 비용 절감 원칙 유지) — Reflection 프롬프트 마지막
줄(`PROPOSED_CHANGE: ...`)만 파싱해 `reflections.proposed_change`에 저장한다
(`core/trading/reflection.py:_extract_proposed_change`).

---

## 구현 현황 — 자동화 범위

아래 단계까지 자동화되어 있다 (`core/trading/self_improvement.py:propose_candidate`):

1. Reflection이 개선안을 제안하면(`PROPOSED_CHANGE:` 파싱) 즉시 백테스트(1Y)를 실행한다.
2. 직전 승인·배포 버전의 `backtest_result`와 비교해 승률·샤프 지수·MDD 중 하나라도 열화되면
   **후보를 저장하지 않고 폐기**한다(원칙 1의 강제 구현).
3. 통과하면 `strategy_versions`에 미승인(`approved_by IS NULL`) 후보로 저장하고
   `version_candidate_ready` 이벤트로 `#stock-system`에 게시한다.
4. 개발자가 `/version approve|reject`로 승인·반려하고, `/version rollback`으로 과거 배포
   이력이 있는 버전으로 즉시 되돌릴 수 있다(`core/api/routes.py`).

**자동화되지 않는 부분**: 실제 `core/trading/prompts/*.md` 프롬프트 문구 수정이나 전략
파라미터 코드 변경은 개발자가 직접 한다 — 승인은 "이 변경을 배포로 기록"하는 것이지
코드/프롬프트 파일을 자동으로 고쳐 쓰는 것이 아니다(하드 금지 사항 "에이전트 프레임워크
금지"와 같은 이유로, 코드 자체를 자율 수정하는 루프는 만들지 않는다).
...
# strategy_versions 테이블 예시
{
  ...
  "backtest_result": {"win_rate": 0.68, "mdd": -0.041, "sharpe_ratio": 1.92, "avg_return": 0.031, "profit_factor": 1.9},
  "approved_by": "discord:Ruthgyeul",
  "proposed_at": "2026-07-19T15:40:00+09:00",
  "deployed_at": "2026-07-20T08:50:00+09:00"
}
```

`approved_by`가 `NULL`이면 승인 대기 후보, 채워지면(`deployed_at`도 함께 채워짐) 배포된
버전이다. 롤백(`/version rollback {strategyVersion}`)은 과거에 배포 이력이 있는 레코드를
찾아 **같은 `strategy_version`/`prompt_version`으로 새 레코드를 추가**하는 방식으로
동작한다 — 기존 이력을 지우거나 덮어쓰지 않아 감사 추적이 항상 남는다. 실제
`core/trading/prompts/*.md` 파일 교체는 개발자가 직접 수행한다 — 별도 마이그레이션이
필요 없도록 프롬프트·전략 파일은 항상 버전 이름이 파일명에 드러나야 한다
(`system_kr_v3`, `system_kr_v4` ...).
