...
이 봇은 **단일 개발자 전용**이다. `discord-bot/src/index.ts`의 `interactionCreate`
핸들러가 모든 슬래시 명령 실행 전에 `interaction.user.id === DISCORD_DEVELOPER_ID`인지
확인하고, 일치하지 않으면 어떤 명령이든(조회 명령 포함) 실행하지 않고 "권한 없음" Embed로
응답한다. 이는 Discord 길드의 역할·권한 설정과 완전히 독립적으로 코드 레벨에서 강제되는
1차 방어선이다 — 길드 관리자 역할을 여러 명이 갖고 있어도 `DISCORD_DEVELOPER_ID`가 아니면
막힌다.

추가 방어선으로, 자금·주문·운영 모드·배포 상태를 바꾸는 명령(`/buy`, `/sell`, `/cancel`,
`/stop`, `/resume`, `/simulate`, `/dryrun`, `/watchlist add|remove`,
`/version approve|reject|rollback`)은 `SlashCommandBuilder.setDefaultMemberPermissions
(PermissionFlagsBits.Administrator)`로 Discord 플랫폼 레벨의 기본값도 관리자 전용으로
제한한다 — 봇 코드에 결함이 있어도 Discord 자체가 한 번 더 막아준다.
