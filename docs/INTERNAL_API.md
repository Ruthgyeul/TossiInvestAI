...
| `POST /api/v1/reports/generate` | `/report`, `/report kr\|us` | `report.ts` | `{ market?: "KR"\|"US"\|"ALL" }` | `202 { jobId }` → 이후 `report_ready` 이벤트로 `ReportEmbedData`(`embeds/report.ts`) + `chartPaths`(그래프 PNG) + `htmlPath`(자기완결 HTML 문서) 전달 |
...
| `report_ready` | `buildReportEmbed` (`embeds/report.ts`, payload = `ReportEmbedData` + `chartPaths`·`htmlPath`) | `#stock-analyze` — 또는 `correlation_id`가 일치하는 `/report` 인터랙션을 edit. Embed(마크다운) + 그래프 PNG + HTML 문서(`htmlPath`, 있으면)를 파일로 첨부 |
