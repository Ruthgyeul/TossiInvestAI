...
- `/report`, `/backtest` → 즉시 `202 {"jobId": "..."}` 반환. discord-bot은
  ...
  마무리한다.
- 나머지 11개 명령(`/status`, `/holdings`, `/orders`, `/buy`, `/sell`, `/cancel`,
  `/stop`, `/resume`, `/simulate`, `/dryrun`, `/simstatus`, `/fund`, `/apicost`,
  `/watchlist`, `/health`, `/version`)은 동기 응답으로 충분하다.
...
| `POST /api/v1/control/stop` | `/stop`, `/stop kr\|us` | `stop.ts` | `{ market?: "KR"\|"US" }` | `{ success: boolean, emergencyStop: boolean, krStop: boolean, usStop: boolean }` |
| `POST /api/v1/control/resume` | `/resume` | `resume.ts` | `{}` | `{ success: boolean }` |
| `POST /api/v1/control/simulate` | `/simulate on\|off` | `dryrun.ts` | `{ state: "on"\|"off" }` | `{ success: boolean, simulation: boolean }` |
...
| `GET /api/v1/version` | `/version` | `version.ts` | — | `{ strategyVersion, promptVersion, deployedAt }` (`strategy_versions` 테이블) |
...
```json
{
  "event_type": "trade_executed | safety_rejection | emergency_stop | health_alert | report_ready | backtest_complete | status_update",
