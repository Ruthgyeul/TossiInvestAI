...
# 키오스크 모니터 — 자체 .env 필요 (MONITOR_SESSION_SECRET 등, monitor/README.md 참고)
(cd monitor && cp .env.example .env && npm install)
```

### 실행
각각 별도 터미널(또는 systemd 서비스, `docs/DEPLOYMENT.md`)로 띄운다.
```bash
